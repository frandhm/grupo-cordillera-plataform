...
export function DashboardPage({ token, onLogout }: Props) {
  ...
  const emptyMeta: CreateMetaPayload = { nombre: '', areaId: '', indicadorId: '', periodo: '', fechaInicio: '', fechaFin: '', valorObjetivo: 0, operador: '>=' as OperadorMeta, unidad: '', descripcionObjetivo: '' };
  ...
  const handleEditarMeta = (meta: Meta) => {
    setMetaForm({
      ...
      unidad: meta.unidad,
      descripcionObjetivo: meta.descripcionObjetivo || ''
    });
